...
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   1. Какие поля IP-дейтаграммы всегда изменяются от одной дейтаграммы к
      ...
      компьютером (последовательность – это все сообщения, которые отправляются до конечного хоста)? **Identification, ttl**
      - <img src="images/task1_img4.png" width=600 />
      - <img src="images/task1_img5.png" width=600 />
   2. Какие поля не меняются? **src, dst, protocol, version, header length, flags** Какие поля должны оставаться неизменными? **dst, scr, protocol** Какие поля должны изменяться? (в рамках одного запуска утилиты) **Identification, ttl**
   3. Как изменяется значение поля Identification IP-дейтаграммы с каждым
      последующим эхо-запросом? Есть ли какая-либо закономерность? **+1**

5. Какое значение содержится в поле Identification (Идентификация), а какое – в поле TTL
   (выберите какой-либо один пакет ICMP)? **Identification: 0x4178, ttl: 1**
6. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   ближайшего (первого транзитного) маршрутизатора. **В ответном сообщении Identification исходного сообщения будет такой же, а ttl = 1**
   - <img src="images/task1_img6.png" width=600 />
7. Найдите серии откликов ICMP, в которых содержатся сообщения об истечении
   ...
   Какое значение содержится в поле Identification (Идентификация), а какое – в поле TTL? ** Identification: 0xf719, ttl: 64**
   - <img src="images/task1_img7.png" width=600 />
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   1. Было ли это сообщение фрагментировано между двумя или более IP-дейтаграммами? **Да**
